...
export type VFileInputSlots = VInputSlots &
  VFieldSlots & {
    counter: never;
    selection: {
      fileNames: string[];
...
export const VFileInput = genericComponent<VFileInputSlots>()({
  ...
  setup(props, { attrs, emit, slots }) {
    ...
    const {outlineStyles} = useOutline({
      style: "dashed",
      width: "2px",
      color: props.dragBorderColor
    })
    ...
    function truncateText(str: string) {
      ...
      const charsKeepOneSide = Math.floor(
        (Number(props.truncateLength) - 1) / 2
      );
    ...
    useRender(() => {
      ...
      const expectsDirectory =
        attrs.webkitdirectory !== undefined && attrs.webkitdirectory !== false;
      const inputAccept = expectsDirectory
        ? undefined
        : props.filterByType ?? String(attrs.accept);

      return (
        <>
        <p>{outlineStyles.value["outline-color"]}</p>
          <VInput
            ref={vInputRef}
            ...
            style={[
              ...
              /*{
                ...
                "--v-file-input-drag-background-opacity":
                  props.dragBackgroundOpacity,
              },*/
              isDragging.value && outlineStyles.value
            ]}
                          ...
                          {!!model.value?.length &&
                            ...
                            (slots.selection
                              ...
                              ? fileNames.value.map((text) => (
                                  <VChip key={text} size="small" text={text} />
                                ))
